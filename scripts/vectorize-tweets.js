  ...
  for (const tweet of tweets) {
    ...
    if (type == 'quote-tweet' && quoted_tweet) {
      context = `Quote tweet from @${username} (User ID: ${user_id}) with ${reply_count} replies, ${retweet_count} retweets, and ${favorite_count} favorites.
        ...
        We also checked if ${username} mentioned other authors and found ${JSON.stringify(
        user_mentions.join(', '),
      )}. 
        Content of our quote: "${full_text}". 
        
        Quoting a tweet from @${quoted_tweet.username} (User ID: ${
        quoted_tweet.user_id
      }) with ${quoted_tweet.reply_count} replies, ${
        quoted_tweet.retweet_count
      } retweets, and ${quoted_tweet.favorite_count} favorites. 
        Details of the tweet we quoted > Content: "${
          quoted_tweet.full_text
        }". Created at: ${quoted_tweet.created_at}. View count: ${
        quoted_tweet.view_count
      }. Quote count: ${quoted_tweet.quote_count}.
      ...
      type == 'thread-tweet' &&
      replying_to_tweet.match(tweetURLPattern)
    ) {
      const [tweetURL, replyToAuthor, statusId] =
        replying_to_tweet.match(tweetURLPattern);
    ...
    try {
      ...
      console.log(
        ...
        `A valid table has been found 🎉 for ${url}`,
      );
    } catch (error) {
...
const jsonFilePath =
  'data/parsed/dataset_twitter_scraper_with_replies_parsed.json';
